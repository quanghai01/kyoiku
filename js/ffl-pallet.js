 ...
 */
  ...
  var isCreateJS = false; // CreateJS 判定フラグ
  ...
  (function(global, $) {
      ...
      var init = {
          ...
          handler: function() {
              ...
              // 自身が iframe 内で参照されているかどうか
              if(window != window.parent) { isInFrame = true; }
              ...
              document.documentElement.addEventListener('touchend', function (e) {
                  ...
                  // CreateJS でタッチイベントの二重発動防止
                  if(e.target.nodeName == 'CANVAS' && isInFrame && isCreateJS)
                  {
                      e.preventDefault();
                  }
              }, false);
  ...
  var getPos = function (e) {
      var posP = {x: 0, y: 0};
      return e.touches && e.touches[0] ? posP = {
          x: e.touches[0].clientX,
          y: e.touches[0].clientY
      } : e.originalEvent && e.originalEvent.changedTouches && e.originalEvent.changedTouches[0] ? posP = {
          x: e.originalEvent.changedTouches[0].clientX,
          y: e.originalEvent.changedTouches[0].clientY
      } : e.clientX && e.clientY && (posP = {x: e.clientX, y: e.clientY}), posP
  }
  ...
  const hitTest = function(sx, sy, ex, ey, x, y, threshold, tool)
  {
      if(tool == TOOL_PEN)
      {
          ...
          if( rect_sx < 0 && 0 < rect_ex && rect_sy + threshold >= 0 && rect_sy - threshold <= 0 ){
              return true;
          }
          return false;
      }
