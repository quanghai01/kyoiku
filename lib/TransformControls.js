...
THREE.TransformControls = function ( camera, domElement ) {
	...
	this.setRotationAxis = function (setAxis){
		this.axis = setAxis;
		absoluteAxis = setAxis;
	};
	...
	this.pointerMove = function ( pointer ) {
		...
		if ( mode === 'translate' ) {
			...
			if ( axis === 'E' ) {
				...
				rotationAxis.copy( _unit[ 'X' ] );

				_tempVector.copy( _unit[ 'X'  ] );
				...
				rotationAxis.copy( _unit[ 'Y' ] );
				_tempVector.copy( _unit[ 'Y' ] );
...
THREE.TransformControlsGizmo = function () {
	...
	matLineRed.opacity = 0;

	var matLineGreen = gizmoLineMaterial.clone();
	// matLineGreen.color.set( 0x00ff00 );
	matLineGreen.opacity = 0;

	var matLineBlue = gizmoLineMaterial.clone();
	// matLineBlue.color.set( 0x0000ff );
	matLineBlue.opacity = 0;

	var matLineCyan = gizmoLineMaterial.clone();
	// matLineCyan.color.set( 0x00ffff );
	matLineCyan.opacity = 0;

	var matLineMagenta = gizmoLineMaterial.clone();
	// matLineMagenta.color.set( 0xff00ff );
	matLineMagenta.opacity = 0;

	var matLineYellow = gizmoLineMaterial.clone();
	// matLineYellow.color.set( 0xffff00 );
	matLineYellow.opacity = 0;

	var matLineGray = gizmoLineMaterial.clone();
	// matLineGray.color.set( 0x787878 );
	matLineGray.opacity = 0;
